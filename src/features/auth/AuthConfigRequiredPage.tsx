    ...
    <main className="mx-auto flex min-h-screen w-full max-w-lg items-center px-4 py-6">
      <section className="w-full rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
        ...
        <p className="mt-2 text-sm text-slate-600">
          Login/registration are enabled, but Supabase is not configured correctly.
        </p>
        {supabaseConfigIssue && (
        ...
        <p className="mt-3 text-xs text-slate-500">
          Add these values to your `.env` file. For Docker/VPS deploys, rebuild the
          container because Vite embeds these values during the production build.
        </p>
      </section>
    </main>
  );
